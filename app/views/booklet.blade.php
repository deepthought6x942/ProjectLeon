...
@section('content')
    ...
    ?>
    <h1><strong>
            <center>Live Auction Items</center>
        </strong></h1>
    <ol>
    ...
    @foreach($donationsTable as $donation)
        ...
        @if ($currentLocation!==$donation->location)
            <h1><strong>
                    <center>{{$currentLocation=$donation->location}}</center>
                </strong></h1>
            <h2><strong>
                    <center>{{$currentCategory=$donation->category}}</center>
                </strong></h2>
        @elseif ($currentCategory!==$donation->category)
            <h3>
                <center><strong>{{$currentCategory=$donation->category}}</center>
                </strong></h3>
        @endif
